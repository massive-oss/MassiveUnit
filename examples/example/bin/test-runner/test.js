...
massive.munit.async.AsyncDelegate = function(testCase,handler,delegateHandler,timeout,info) { if( testCase === $_ ) return; {
	...
	this.timer = massive.munit.util.Timer.delay($closure(this,"timeoutHandler"),timeout);
}}
...
massive.munit.async.AsyncDelegate.prototype.handlerName = function() {
	var fields = Reflect.fields(this.testCase);
	{
		var _g = 0;
		while(_g < fields.length) {
			var field = fields[_g];
			++_g;
			if(Reflect.field(this.testCase,field) == this.handler) {
				return field;
			}
		}
	}
...
massive.munit.MUnitException = function(message,info) { if( message === $_ ) return; {
	this.type = { fileName : "MUnitException.hx", lineNumber : 17, className : "massive.munit.MUnitException", methodName : "new"}.className;
...
massive.munit.UnhandledException = function(message,info) { if( message === $_ ) return; {
	...
	this.type = { fileName : "UnhandledException.hx", lineNumber : 14, className : "massive.munit.UnhandledException", methodName : "new"}.className;
}}
...
massive.munit.TestRunner.prototype.execute = function() {
	{
		...
		while(_g1 < _g) {
			...
			{ var $it0 = suite;
			while( $it0.hasNext() ) { var testCaseType = $it0.next();
			{
				if(this.activeHelper == null || this.activeHelper.type != testCaseType) {
					this.activeHelper = new massive.munit.TestCaseHelper(testCaseType);
...
massive.munit.TestRunner.prototype.executeTestCases = function() {
	...
	{
		this.testCount++;
		this.activeHelper.before.apply(this.activeHelper.test,new Array());
...
massive.munit.TestRunner.prototype.run = function(testSuiteTypes) {
	...
	{
		var _g = 0;
		while(_g < testSuiteTypes.length) {
			var suiteType = testSuiteTypes[_g];
			++_g;
...
massive.munit.async.MissingAsyncDelegateException = function(message,info) { if( message === $_ ) return; {
	massive.munit.MUnitException.apply(this,[message,info]);
	this.type = { fileName : "MissingAsyncDelegateException.hx", lineNumber : 15, className : "massive.munit.async.MissingAsyncDelegateException", methodName : "new"}.className;
}}
...
massive.munit.client.HTTPClient = function(client,url,httpRequest) { if( client === $_ ) return; {
	this.client = client;
	this.url = url;
...
massive.munit.client.HTTPClient.prototype.onData = function(data) {
	haxe.Log.trace("\nonData: " + data,{ fileName : "HTTPClient.hx", lineNumber : 62, className : "massive.munit.client.HTTPClient", methodName : "onData"});
}
massive.munit.client.HTTPClient.prototype.onError = function(msg) {
	haxe.Log.trace("\nonError: " + msg,{ fileName : "HTTPClient.hx", lineNumber : 67, className : "massive.munit.client.HTTPClient", methodName : "onError"});
}
massive.munit.client.HTTPClient.prototype.onStatus = function(status) {
	haxe.Log.trace("\nonStatus: " + status,{ fileName : "HTTPClient.hx", lineNumber : 72, className : "massive.munit.client.HTTPClient", methodName : "onStatus"});
}
massive.munit.client.HTTPClient.prototype.reportFinalStatistics = function(testCount,passCount,failCount,errorCount,time) {
	...
	haxe.Log.trace(result,{ fileName : "HTTPClient.hx", lineNumber : 43, className : "massive.munit.client.HTTPClient", methodName : "reportFinalStatistics"});
	return result;
}
massive.munit.client.HTTPClient.prototype.sendResult = function(result) {
	if(this.httpRequest == null) {
		...
		this.httpRequest.onStatus = $closure(this,"onStatus");
	}
	this.httpRequest.setParameter("data",result);
	this.httpRequest.request(true);
}
...
massive.munit.AssertionException = function(msg,info) { if( msg === $_ ) return; {
	...
	this.type = { fileName : "AssertionException.hx", lineNumber : 14, className : "massive.munit.AssertionException", methodName : "new"}.className;
}}
...
SampleTest.prototype.testConstructor = function(asyncFactory) {
	var handler = asyncFactory.createBasicHandler(this,$closure(this,"onTestConstructor"),300,{ fileName : "SampleTest.hx", lineNumber : 43, className : "SampleTest", methodName : "testConstructor"});
...
TestMain = function(p) { if( p === $_ ) return; {
	...
	var runner = new massive.munit.TestRunner(new massive.munit.client.HTTPClient(new massive.munit.client.JUnitReportClient(),"http://localhost:2000/"));
...
TestMain.prototype.completionHandler = function(success) {
	js.Lib.eval(("testResult(" + success) + ");");
}
TestMain.prototype.__class__ = TestMain;
massive.munit.async.AsyncTimeoutException = function(message,info) { if( message === $_ ) return; {
	massive.munit.MUnitException.apply(this,[message,info]);
	this.type = { fileName : "AsyncTimeoutException.hx", lineNumber : 14, className : "massive.munit.async.AsyncTimeoutException", methodName : "new"}.className;
}}
...
massive.munit.async.delegate.AsyncDispatcherDelegate = function(testCase,handler,timeout,info) { if( testCase === $_ ) return; {
	massive.munit.async.AsyncDelegate.apply(this,[testCase,handler,$closure(this,"asyncHanlder"),timeout,info]);
}}
massive.munit.async.delegate.AsyncDispatcherDelegate.__name__ = ["massive","munit","async","delegate","AsyncDispatcherDelegate"];
massive.munit.async.delegate.AsyncDispatcherDelegate.__super__ = massive.munit.async.AsyncDelegate;
for(var k in massive.munit.async.AsyncDelegate.prototype ) massive.munit.async.delegate.AsyncDispatcherDelegate.prototype[k] = massive.munit.async.AsyncDelegate.prototype[k];
massive.munit.async.delegate.AsyncDispatcherDelegate.prototype.asyncHanlder = function(message,target) {
	if(this.timedOut) return;
	this.timer.stop();
	this.params = [message,target];
	if(this.observer != null) this.observer.asyncExecuteHandler(this);
}
massive.munit.async.delegate.AsyncDispatcherDelegate.prototype.message = null;
massive.munit.async.delegate.AsyncDispatcherDelegate.prototype.target = null;
massive.munit.async.delegate.AsyncDispatcherDelegate.prototype.__class__ = massive.munit.async.delegate.AsyncDispatcherDelegate;
...
massive.munit.async.AsyncFactory.prototype.createDispatchHandler = function(testCase,handler,timeout,info) {
	var dispatcher = new massive.munit.async.delegate.AsyncDispatcherDelegate(testCase,handler,timeout,info);
	dispatcher.observer = this.observer;
	this.asyncDelegateCount++;
	return dispatcher.delegateHandler;
}
...
massive.munit.TestCaseHelper = function(type) { if( type === $_ ) return; {
	this.type = type;
	this.tests = new Array();
	this.index = 0;
...
massive.munit.TestCaseHelper.__name__ = ["massive","munit","TestCaseHelper"];
massive.munit.TestCaseHelper.prototype.after = null;
massive.munit.TestCaseHelper.prototype.afterClass = null;
massive.munit.TestCaseHelper.prototype.before = null;
massive.munit.TestCaseHelper.prototype.beforeClass = null;
massive.munit.TestCaseHelper.prototype.className = null;
massive.munit.TestCaseHelper.prototype.current = function() {
	return ((this.index <= 0)?this.tests[0]:this.tests[this.index - 1]);
}
massive.munit.TestCaseHelper.prototype.hasNext = function() {
	return this.index < this.tests.length;
}
massive.munit.TestCaseHelper.prototype.index = null;
massive.munit.TestCaseHelper.prototype.next = function() {
	return (this.hasNext()?this.tests[this.index++]:null);
}
massive.munit.TestCaseHelper.prototype.nullFunc = function() {
	null;
}
massive.munit.TestCaseHelper.prototype.parse = function(type) {
	...
	if(meta == null) return;
	{
		...
		while(_g < fields.length) {
			...
			if(Reflect.isFunction(f) && Reflect.hasField(meta,field)) {
				...
				{
					var _g1 = 0, _g2 = massive.munit.TestCaseHelper.META_TAGS;
					while(_g1 < _g2.length) {
						...
						if(Reflect.hasField(funcMeta,tag)) {
							...
							case "Test":{
								...
								result.name = field;
								this.tests.push({ test : f, scope : this.test, result : result});
							}break;
							}
						}
					}
...
massive.munit.TestCaseHelper.prototype.test = null;
massive.munit.TestCaseHelper.prototype.tests = null;
massive.munit.TestCaseHelper.prototype.type = null;
massive.munit.TestCaseHelper.prototype.__class__ = massive.munit.TestCaseHelper;
massive.munit.client.PrintClient = function(p) { if( p === $_ ) return; {
	this.init();
	this.print("MUnit v0.1" + this.newline);
}}
...
massive.munit.client.PrintClient.prototype.init = function() {
	...
	if(this.textArea == null) {
		var error = ((((("MissingElementException: 'haxe:trace' element not found at " + { fileName : "PrintClient.hx", lineNumber : 58, className : "massive.munit.client.PrintClient", methodName : "init"}.className) + "#") + { fileName : "PrintClient.hx", lineNumber : 58, className : "massive.munit.client.PrintClient", methodName : "init"}.methodName) + "(") + { fileName : "PrintClient.hx", lineNumber : 58, className : "massive.munit.client.PrintClient", methodName : "init"}.lineNumber) + ")";
		js.Lib.alert(error);
	}
}
...
massive.munit.client.PrintClient.prototype.reportFinalStatistics = function(testCount,passCount,failCount,errorCount,time) {
	...
	this.print((((((((((this.newline + "Tests: ") + testCount) + "  Passed: ") + passCount) + "  Failed: ") + failCount) + " Errors: ") + errorCount) + " Time: ") + massive.munit.util.MathUtil.round(time,5));
...
massive.munit.TestResult.prototype.toString = function() {
	var s = "";
	if(this.passed) null;
	else if(this.failure != null) null;
	else if(this.error != null) null;
	return s;
}
...
js.Lib.onerror = null;
massive.munit.TestCaseHelper.META_TAG_BEFORE_CLASS = "BeforeClass";
massive.munit.TestCaseHelper.META_TAG_AFTER_CLASS = "AfterClass";
massive.munit.TestCaseHelper.META_TAG_BEFORE = "Before";
massive.munit.TestCaseHelper.META_TAG_AFTER = "After";
massive.munit.TestCaseHelper.META_TAG_TEST = "Test";
massive.munit.TestCaseHelper.META_TAG_TEST_ASYNC = "Async";
massive.munit.TestCaseHelper.META_TAGS = ["BeforeClass","AfterClass","Before","After","Test","Async"];
